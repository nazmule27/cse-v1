...
class Research_model extends CI_Model
{
    ...
    public function getResearchPublication($research_id) {
        $query=$this->db->query("SELECT distinct pid, title, c_j_name, IF(no = '', vol, CONCAT(vol, '(', no, ')')) AS vol, page, year, doi FROM pub WHERE pid IN (SELECT pid FROM pub_area 
WHERE research_id IN ( SELECT research_id FROM research WHERE research_id=$research_id OR parentid=$research_id )) ORDER BY year DESC ");
        return $result = $query->result();
    }
    ...
    public function getPublication($year) {
        ...
        $query=$this->db->query("SELECT distinct n.pid, m.authors, n.title, n.c_j_name, IF(n.no = '', vol, CONCAT(vol, '(', no, ')')) AS vol, n.page, n.year, n.doi  FROM
(SELECT x.pubid, GROUP_CONCAT(' ', x.authors ORDER BY rank) AS authors FROM 
(SELECT rank, b.pubid, CONCAT(a.firstname, ' ', a.lastname) AS authors,  a.aid, a.auid  FROM authors a, author_pub b  WHERE a.aid=b.aid) x GROUP BY x.pubid) m, pub n, (SELECT ap.pubid FROM author_pub ap, authors a WHERE ap.aid=a.aid) o WHERE m.pubid=o.pubid AND m.pubid=n.pid $year_string ORDER BY n.year DESC");
        return $result = $query->result();
    }
